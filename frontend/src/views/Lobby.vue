<template>
  <div id="container">
    <VoteCardContainer
      v-bind:valid-votes="validVotes"
      v-bind:issue-status="issueStatus"
      @emit-vote="emitVote"
    />
    <h3 v-if="this.issueStatus === 'Voting'">Voting issue #{{ issue }}</h3>
    <h3 v-else>
      Issue #{{ issue }} voted
      <span v-show="avg !== null">• Average: {{ avg }}</span>
    </h3>
    <button v-show="this.issueStatus === 'Finished'" @click="$router.push('/')">
      Join another issue
    </button>
    <h3>Connected {{ members.length }}</h3>
    <MembersContainer
      v-bind:members="this.members"
      v-bind:issueStatus="this.issueStatus"
    />
...
export default {
  ...
  methods: {
    ...
    async emitVote(vote) {
      ...
      if (isError(status)) {
        await this.$store.dispatch("error/setError", `Something went wrong`, {
          root: true,
        });
      }
    },
